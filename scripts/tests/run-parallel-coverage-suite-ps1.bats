...
setup() {
  ...
  # Bash shim: records args, emits JSON fixture per subcommand. Derives its own
  # directory instead of reading BATS_TEST_TMPDIR, which does not cross the
  # wsl.exe re-entry below (Windows -> WSL does not propagate the caller's env).
...
@test "run-parallel-coverage-suite.ps1: happy path invokes kmp-test parallel --json" {
  run "$PWSH" -NoProfile -ExecutionPolicy Bypass \
    ...
    2>&1
  [ -f "$ARGS_LOG" ]
  grep -q "parallel" "$ARGS_LOG"
  grep -q "\-\-json" "$ARGS_LOG"
}
...
@test "run-parallel-coverage-suite.ps1: -SkipTests invokes kmp-test coverage subcommand" {
  run "$PWSH" -NoProfile -ExecutionPolicy Bypass \
    ...
    2>&1
  [ -f "$ARGS_LOG" ]
  grep -qE "^coverage|coverage " "$ARGS_LOG"
  ! grep -qE "^parallel|parallel --" "$ARGS_LOG"
}
...
@test "run-parallel-coverage-suite.ps1: -ExcludeCoverage translates to --exclude-modules with deprecation warning" {
  run "$PWSH" -NoProfile -ExecutionPolicy Bypass \
    ...
    2>&1
  [ -f "$ARGS_LOG" ]
  grep -q "\-\-exclude-modules" "$ARGS_LOG"
  ! grep -q "\-\-exclude-coverage" "$ARGS_LOG"
  [[ "$output" == *"deprecated"* ]]
}
