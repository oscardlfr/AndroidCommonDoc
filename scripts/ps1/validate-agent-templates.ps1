# Windows PowerShell wrapper for validate-agent-templates.sh
#
# Thin bridge — all logic lives in scripts/sh/validate-agent-templates.sh. This wrapper exists
# for cross-platform script parity (scripts/sh/ ↔ scripts/ps1/). Requires
# bash on PATH (Git for Windows provides it).
#
# Validate setup/agent-templates/ frontmatter and body rules
...
$shScript = Join-Path $scriptsRoot 'sh' 'validate-agent-templates.sh'
