# Windows PowerShell wrapper for install-git-hooks.sh
#
# Thin bridge — all logic lives in scripts/sh/install-git-hooks.sh. This wrapper exists
# for cross-platform script parity (scripts/sh/ ↔ scripts/ps1/). Requires
# bash on PATH (Git for Windows provides it).
#
# Install project git hooks to .git/hooks/
...
$shScript = Join-Path $scriptsRoot 'sh' 'install-git-hooks.sh'
