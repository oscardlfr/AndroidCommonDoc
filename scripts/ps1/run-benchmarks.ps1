...
if ($IncludeShared) {
    $sharedRoot = Join-Path (Split-Path $ProjectRoot -Parent) "shared-kmp-libs"
    if (Test-Path $sharedRoot) {
        ...
        foreach ($sm in $sharedModules) {
            $modules += "shared-kmp-libs:$sm"
        }
    } else {
        Write-Host "[!!] shared-kmp-libs directory not found at: $sharedRoot" -ForegroundColor Yellow
    }
}
...
foreach ($mod in $modules) {
    ...
    $isShared = $mod.StartsWith("shared-kmp-libs:")
    if ($isShared) {
        $effectiveRoot  = Join-Path (Split-Path $ProjectRoot -Parent) "shared-kmp-libs"
        $effectiveModule = $mod -replace '^shared-kmp-libs:', ''
    } else {
