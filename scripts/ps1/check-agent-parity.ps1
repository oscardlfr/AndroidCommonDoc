# Windows PowerShell wrapper for check-agent-parity.sh
#
# Thin bridge — all logic lives in scripts/sh/check-agent-parity.sh. This wrapper exists
# for cross-platform script parity (scripts/sh/ ↔ scripts/ps1/). Requires
# bash on PATH (Git for Windows provides it).
#
# Check .claude/agents/ vs GSD subagent/ parity
...
$shScript = Join-Path $scriptsRoot 'sh' 'check-agent-parity.sh'
