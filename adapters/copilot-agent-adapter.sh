...
# Remove only stale files previously generated by this adapter. Hand-written
# files are never deleted: an orphan without the exact generator marker is
# left for the parity validator to report for manual disposition.
for output_file in "$OUTPUT_DIR"/*.agent.md; do
  [ -f "$output_file" ] || continue
  agent_name="$(basename "$output_file" .agent.md)"
  [ -f "$CLAUDE_AGENTS_DIR/$agent_name.md" ] && continue
  if head -n 1 "$output_file" | grep -Fqx "<!-- GENERATED from .claude/agents/$agent_name.md -- DO NOT EDIT MANUALLY -->"; then
    rm -f -- "$output_file"
    echo "  Cleaned generated orphan: $(basename "$output_file")"
  fi
done
