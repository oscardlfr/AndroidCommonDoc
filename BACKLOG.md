...
**LOW — `PA-JS1`/`PA-JS2` assert only `"BLOCKED"` in output.** They do not pin `exit 2` and `decision: block`. A crash also produces non-zero exit; a partial write could produce the substring. Flagged by `arch-testing` during review, acknowledged, never filed. Same class as `#PAG-PEER-BLOCK`'s vacuity — an assertion weaker than the property it names.
