...
Currently dormant elsewhere: `scripts/tests/bats-handoff.bats` and `scripts/tests/emit-push-proof.bats` are clean, and `scripts/tests/run-bats.bats` is clean only because `#RB13`–`#RB18` all happen to pass `--project-root` explicitly. Dormant is not fixed.
