...
- **D5** (OPEN — see Wave C below) — `pre_pr_coverage` and `discovered_rules` have zero producers repo-wide; `emit-push-proof.sh` still only checks their *presence*, not that the content is real. Explicitly out of Wave A's scope.
...
- **`docs/agents/quality-gater-secret-scan.md:49,55`** — blanket claim `"quality-gate-manifest.json — NOT touched"`. The specific point (secret-scan needs no manifest entry) stays true, but the unscoped wording is now false: Step 9 does touch the manifest, for an unrelated reason (`informational_steps`). Same defect shape as the freshness-gate lines already fixed in this wave, lower severity.
...
**Wave C — `pre_pr_coverage`/`discovered_rules` producer + binding (BLOCKING for Wave 5's resume).** `feature/portable-ingestion-wave38` (Wave 5) must not resume until a wave lands a real producer for `pre_pr_coverage` and `discovered_rules` — today `emit-push-proof.sh` only checks that these keys are non-empty/present, not that their content reflects a real `/pre-pr` run or real rule discovery. Explicitly dropped from Wave A's scope (its asserted shape matched neither the honest report on disk nor the existing green fixtures); direct successor to Wave A in the B → A → C → Wave 5 sequence.
...
**HIGH — `APPROVED-VERIFY-FINAL` is an unbacked assertion, same defect class as D0 and as the `APPROVED-PREP` item above, one phase later and one layer more load-bearing.** `scripts/sh/write-verdict.sh` verify-final reads the architect's body from stdin with a bare `stdin_content="$(cat)"` — it accepts anything, including an empty body. `scripts/sh/emit-push-proof.sh:562-578` then gates the mint on three checks only: an `arch-*-verdict.md` exists; it contains the literal string `APPROVED-VERIFY-FINAL`; its `**HEAD**:` matches final HEAD. **The body is never read.** So the token proves an architect ran a script and supplied *some* text — never that it reviewed anything, and never that any claim inside it is backed. Unlike `APPROVED-PREP`, which unlocks specialist execution, this token unlocks the **push**.

Observed in Wave A (2026-07-09): a signed VERIFY-FINAL asserted `1945 ok, 0 not ok` sourced from a chat report, at a HEAD where no bats handoff existed — every suite run that evening had bypassed `scripts/sh/run-bats.sh` (which emits the evidence file) in favour of direct `npx bats` invocation. The mint's own evidence binding would have rejected that data twice (wrong `BATS_HEAD`, absent `BATS_SCOPE`); the verdict gate could not see it at all.

Desired fix: bind `APPROVED-VERIFY-FINAL` to structured review evidence, not merely to a token plus a HEAD match. At minimum: (a) reject empty or unstructured verify-final bodies; (b) when the verdict claims test evidence, require it to cite a HEAD-bound, full-scope bats handoff — `BATS_HEAD` == the verdict's `**HEAD**:`, `BATS_SCOPE=full`, `BATS_NOT_OK=0`, `BATS_COMPLETE=true`. Mirror Wave A's §A4: the gate reads evidence, not an assertion. *Explicitly NOT fixed in Wave A, by user decision — it changes both the verdict evidence contract and the push-mint semantics, so it belongs in a dedicated follow-up / Wave C scope. Do not add `write-verdict.sh` to Wave A's Path-Manifest. No silent deferral: this is a named, owned item, not a parked one.*
...
Apply `emit-push-proof.sh`'s §A4 checks to each. **Run 1 satisfies every one** — `status=ok`, `head` matches, `scope=full`, `complete=true`, `not_ok=0`, sanity floor holds — and **mints the proof**. **Run 2 dies `test-suite-evidence-dirty`.** So whether the push gate opened depended on whether anyone had happened to run bats earlier at the same HEAD. Worse: the run the gate would have **blessed** is the one concealing four contaminated tests; the run it would have **blocked** is the one telling the truth about the tree.

`bats_evidence` records **an** answer, not **the** answer. `emit-push-proof.sh` cannot know a second run existed, or that it disagreed.

The *cause* of the divergence was removed in-wave (`df6b52f`, test isolation), so there is no live exposure today. **The gate still cannot detect a non-reproducible suite.**

Desired fix — require reproducibility, not merely presence. Options to evaluate: (a) the mint requires ≥2 handoffs at the same HEAD with identical `ok`/`not_ok`/`expected`; (b) `run-bats.sh` refuses to overwrite a differing same-HEAD handoff and reports the divergence as a first-class failure; (c) `push-proof.json` carries a `reproduced` field bound to two independent runs. Mirror §A4: the gate reads evidence, not an assertion — and *"an answer"* is an assertion about *"the answer."*

*Found by arch-testing, from the raw handoff archive, after the orchestrator's design had already shipped every other check. User-approved for filing 2026-07-09. Explicitly NOT fixed in Wave A — the root cause is fixed; the gate hardening is a tracked follow-up, not a blocker. To be disclosed in the PR body as a known limitation.*
