...
> **Last updated**: 2026-07-07; Harness Realignment Wave 0 (Harness State Ledger Reconciliation, #234 `0bbf6fa`), Wave 1 (Runtime/Topology Contract Realignment, #235 `125409b`), and Wave 2 (Portable Coordination Artifact Layer, #236 `68ed036`) are all MERGED to develop; **Wave 3 — Phase-Orchestration Restoration is the active wave**; RTK template sweep remains deferred pending separate approval
...
- **Wave 3 — Phase-Orchestration Restoration** (**ACTIVE WAVE** — executed DOC: docs/agents wording reconciliation; the planned HARNESS mechanization is deferred to BL-W4-10) — restore "living system by phases" on top of the portable contract.
...
**Sequencing**: Waves 0, 1, 2 are MERGED; **Wave 3 — Phase-Orchestration Restoration is the current active wave**. After Wave 3, Wave 4 (QG/macOS/Local-CI Parity) is next. Recommended minimum Waves 0-5; Wave 6 is optional hardening after Wave 5.
...
- **BL-W4-9** (OPEN, 2026-07-07) — Harden `write-specialist-dispatch.sh:345` confinement prefix check: the `.planning*` bare-glob matches a sibling like `.planning-evil`; needs a trailing-separator / exact-match guard, mirroring the `write-coordination-artifact.sh` fix. Low-severity (only reachable via a symlink planted inside `.planning/`, already a trusted write surface). Surfaced by the Wave 2 Codex-round security re-review.
