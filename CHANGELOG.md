...
- **`pre-push-hook.sh`** (`scripts/sh/`): three-layer push gate — Layer 1: two-stamp validation; Layer 2: QG-proof verification via `emit-push-proof.sh verify-proof`; Layer 3: fallback stamp-only for repos without push-proof.json. Bypass: `SKIP_PUSH_GATE=1` — always logged.
...
- **`docs/guides/pre-commit-hooks.md`**: pre-push row updated from two-stamp to three-layer gate description; frontmatter description updated.
- **`docs/agents/hook-manifest.md`**: git-layer pre-push row updated to describe all three layers.
