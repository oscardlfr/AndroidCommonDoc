...
- **`emit-push-proof.sh` / `emit-push-proof.ps1`** (`scripts/sh/`, `scripts/ps1/`): canonical QG runner + proof emitter/verifier. Two subcommands: `run-qg` attests the quality-gater report (Steps 0-9), enforces verdict→HEAD binding across all arch-*-verdict.md files, and mints `push-proof.json` + backward-compat stamps (`quality-gate.stamp`, `pre-pr.stamp`); `verify-proof` is the cheap git-layer verifier (8 integrity checks: schema, head match, worktree, freshness, manifest-version, step coverage, report digest). Fail-CLOSED on all integrity checks; fail-OPEN only on `push-proof.log` append.
