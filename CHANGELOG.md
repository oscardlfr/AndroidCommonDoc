...
- **F1 — CMP 1.11 Compose UI test API v2** (docs/testing/testing-compose-ui-test-v2.md): NEW doc — v1->v2 migration guide. runComposeUiTest -> runComposeUiTestWithContext, UnconfinedTestDispatcher -> StandardTestDispatcher dispatcher swap, advanceUntilIdle() requirements, effectContext parameter. testing-hub, testing-patterns-coroutines, compose-patterns, compose-semantic-diff:78 updated.
- **F2 — Kotlin 2.4 explicit backing fields** (9 docs): Added field: MutableStateFlow<T> callout alongside existing boilerplate. MutableStateFlowExposedRule.kt verified/extended for explicit-backing-field AST coverage.
