...
parent: agents-hub
category: agents
description: "Request-bound architect verdicts, provenance agreement, QG proof, and post-wave integrity."
version: 2
...
> **Floor vs accelerator**: request-bound `verdict/v1` JSON and provenance evidence on disk are load-bearing. SendMessage tallies and cross-review are optional accelerators and cannot authorize a transition.
...
1. **Every class-required role publishes `arch-{short-role}-verdict-verify-final.json`** through `write-verdict.sh`
...
4. **Validate verdicts**: every required record must bind role, phase, wave, PLAN, final HEAD, immutable request and exact evidence bytes with `decision: approve`
...
└──────────────────────────────────────────────────────┘
  ↓
All required structured verdicts authorize → QG
...
1. **Verify verdict file on disk** with the canonical validator. Missing, stale, malformed or non-authorizing records remain incomplete regardless of the message text.
...
1. Read the canonical `.planning/wave-<slug>/arch-{short-role}-verdict-verify-final.json` for full details.
...
1. Validate every class-required VERIFY-FINAL verdict against the current PLAN, HEAD, request and evidence bytes.
