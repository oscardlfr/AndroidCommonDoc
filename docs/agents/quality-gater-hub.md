...
`emit-qg-result.sh` lifecycle: `--init` at Phase 3 start (writes `status: running`), `--phase <step>` after each step (updates `updated_at`), final write at Step 11 (writes `status: pass|fail` with current HEAD). See [quality-gate-protocol Steps 3 & 11](quality-gate-protocol.md#step-3-test-suite) for step detail.
