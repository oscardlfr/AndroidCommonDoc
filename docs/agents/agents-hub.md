...
| [runtime-messaging-adapters](runtime-messaging-adapters.md) | Wave 1 portable consultation system: disk-artifact floor + adapter contract + orchestrator role — protocol, state-machine, drivers, bridges, cp-writer, modes (6 sub-docs) |
| [runtime-messaging-modes](runtime-messaging-modes.md) | Standalone (Claude-only) vs mixed (Codex worker opt-in) operation, and the preconditions a consumer repository must satisfy for either |
...
| [specialist-dispatch-protocol](specialist-dispatch-protocol.md) | Disk-first specialist↔architect binding: dispatch artifact schema, PREP-/dispatch-currency gate checks, files[] scope, TaskList tracking-only, SendMessage optional-adapter |
