...
| [tl-session-setup](tl-session-setup.md) | team-lead session setup: Phase 2 core devs, selective spawning, rotation protocol, context management, architect routing |
...
- **team-lead** = orchestrator. NEVER codes — orchestrates 3-phase teams, spawns 4 core devs at Phase 2 start, spawns extras on architect request. Pattern validation chain: dev → architect → context-provider.
