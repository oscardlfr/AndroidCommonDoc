...
parent: agents-hub
category: agents
description: "Architect dispatch modes (PREP/EXECUTE) and scope_doc_path protocol. PM includes mode + scope_doc_path in every dispatch; architects read scope from that path (never cwd guess). Fixes Wave 23 Bug #5 (hardcoded PLAN.md) and Bug #6 (missing mode tagging)."
version: 1
last_updated: "2026-04-20"
assumes_read: [pm-phase-execution, pm-dispatch-topology]
token_budget: 1500
---

# Architect Dispatch Modes

This doc defines the two-step architect dispatch protocol introduced in Wave 23. Every PM dispatch to an architect carries two mandatory fields: `scope_doc_path` (the wave plan file) and `mode` (`PREP` or `EXECUTE`). Architects read scope from the provided path and behave differently per mode.

## Problem this solves

**Bug #5 (scope-doc hardcode)**: Arch templates hardcoded `.planning/PLAN.md` in their PRE-TASK and Scope Validation gates. Wave plans live at `.planning/PLAN-W{N}.md` (Wave 22 modularization). Architects reading the hardcoded path either got stale content or failed outright. PM dispatch did not pass a path, so architects guessed from cwd.
...
## Two fields on every PM dispatch

Every PM `SendMessage` to an architect MUST include:
...
Example PM → architect dispatch message body:
...
**When**: PM sends PREP dispatch to all 3 architects **before** devs are spawned for a wave.
...
5. SendMessage PM with `READY: <1-line summary of risks and dev tasks>`
6. Do NOT dispatch devs yet. Do NOT write verdict. Stay idle after READY until EXECUTE dispatch arrives.

**PM collects all 3 READY responses before spawning devs.** This lets PM merge cross-architect concerns into a single dev dispatch plan — e.g., if platform flags a source-set move that testing needs to reconcile, PM surfaces both in the dev brief.

## EXECUTE mode — post-dev verification

**When**: PM sends EXECUTE dispatch to all 3 architects **after** devs have completed their work for the wave.
...
3. Delegate any fixes back to devs via SendMessage to PM (standard flow)
4. Write full verdict block to `.planning/wave{N}/arch-{role}-verdict.md`
5. SendMessage PM with `APPROVE` or `ESCALATE: <1-sentence reason>` (per agent-verdict-protocol.md)

## PM workflow — 2-step dispatch per wave

```
1. PREP dispatch to arch-testing, arch-platform, arch-integration in parallel
   → wait for all 3 READY responses

2. PM merges READY findings, spawns devs (Phase 2 core devs if first wave, extras if requested)
...
5. If all APPROVE → proceed to Phase 3 (quality-gater)
   If any ESCALATE → PM re-plans or dispatches clarification
```
...
1. **Inbox-first**: idle until PM dispatch arrives
...
3. **Path-missing guard**: If `scope_doc_path` is absent or empty in the dispatch message:
   - `SendMessage(to="project-manager", summary="SCOPE-DOC-MISSING", message="Dispatch for wave {N} did not include scope_doc_path. Re-dispatch with the field populated.")`
...
5. **Cross-check**: if the dispatch summary conflicts with scope_doc_path contents, SendMessage PM with `PLAN-DISPATCH DRIFT` and quote both. Do NOT silently follow either.
...
- Starting dev dispatch in PREP mode — PREP is plan-review only. Dev dispatch happens after PM sees all 3 READY responses.
...
- PM spawning devs before all 3 READY responses arrive — ignores cross-architect risk merging.

## Example: Wave 23 Step 5+6 dispatch

Actual PREP dispatch PM would send for Wave 23 Step 5+6:
...
message: Read .planning/PLAN-W23.md Steps 5+6. Target files: 3 arch templates (setup/ + .claude/agents/ copies) + PM template + new docs/agents/arch-dispatch-modes.md. Line budget: arch-testing=396, arch-platform=387, arch-integration=377. Identify KMP/template structural risks. Return READY with dev task list.
```

## See also

- [PM Phase Execution](pm-phase-execution.md) — overall 3-phase protocol
- [PM Dispatch Topology](pm-dispatch-topology.md) — pre-dispatch gate and kill order
