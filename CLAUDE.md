...
- **BL-W32-06e SHIPPED** (2026-05-02, PR #100 feature/bl-w32-06e-cleanup) — Script deduplication cleanup with kmp-test-runner v0.7.0. Thin-wrapped 2 of 5 audited script pairs (parallel-suite + changed-modules; sh+ps1 = 4 files). Reduction ~80% on targeted pairs (3086 → 618 lines saved); ~40% across audited 5-script set (per user-confirmed Option A — 3 KEEP scripts have <5% runner overlap with mechanical justification: build-run-app=app-deploy, run-android-tests=ADB+logcat orchestration, run-benchmarks=benchmark-domain). v0.7.0 bump additive (iOS/macOS NOT shipped; arrives v0.8.0 with full Node migration). 22 bats cases (5+ per script target, test-infra mocked, dry-run Strategy A on parallel-suite, Strategy B on changed-modules --show-modules-only). 13 architect amendments + 1 arch-testing post-dispatch addendum (3 platform + 5 integration + 5 testing + 1 dry-run-echo) all applied. quality-gater 9/9 PASS. BL-W32-12 filed (architect-bash-write-gate `pathlib.Path.write_text()` exempt-target gap surfaced live — first incident data; BL-W32-05 had deferred this detector).
- **BL-W32 Phase 1 SHIPPED** (2026-05-01, PR #97 feature/bl-w32-phase1) — 3 L0-only hardening fixes. **BL-W32-02**: `docs/agents/arch-topology-protocols.md` gains `## 4. Concern Ownership` table (arch-integration owns CI/runtime/wiring; arch-platform owns lib/interface/schema/API; arch-testing owns test design+coverage; tiebreaker chain documented). All 3 architect templates bumped: arch-platform 1.19.0→1.20.0, arch-integration 1.18.0→1.19.0, arch-testing 1.21.0→1.22.0; each `## Role` gained `**Concern ownership**` prose linking to the new table. **BL-W32-03**: `setup/agent-templates/arch-testing.md` gains `### Pre-Dispatch Decision: Mocked vs Fixture-driven` section (BEFORE Dev Routing Table) — 3 decision inputs (existing infra via CP query, test surface type, house-style precedent), `test-infra: mocked/fixture-driven` dispatch token required. **BL-W32-04**: `docs/agents/main-agent-orchestration-guide.md` gains `## Phase 0 — Session start` — TeamDelete-on-reuse rule with CP zombie rationale (`~/.claude/teams/{slug}/config.json` stale `members[]`), Option A deferred. MIGRATIONS entries added for all 3 architects. 7 stale version assertions updated in 2 test files + `feature-domain-specialist` manifest hash. Phase 1 CLOSED. Phase 2 next: L0→L1/L2 sync.
