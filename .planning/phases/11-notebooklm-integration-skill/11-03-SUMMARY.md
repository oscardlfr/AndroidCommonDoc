...
plan: 03
subsystem: vault
tags: [obsidian, vault-sync, moc-generator, vault-writer, sync-engine, typescript, vitest]

# Dependency graph
requires:
  - phase: 11-notebooklm-integration-skill
    plan: 01
    provides: "VaultConfig, VaultSource, VaultEntry, SyncResult, SyncManifest types; collectAll; loadVaultConfig/saveVaultConfig"
  - phase: 11-notebooklm-integration-skill
    plan: 02
    provides: "transformAll: VaultSource[] -> VaultEntry[] with enriched frontmatter and wikilinks"
provides:
  - "generateAllMOCs: 6 index pages (All Patterns, By Project, By Layer, By Target Platform, All Skills, All Decisions)"
  - "writeVault: file I/O with hash-based change detection, .obsidian/ config, manifest tracking"
  - "loadManifest/saveManifest: sync manifest persistence for incremental sync"
  - "detectOrphans: finds stale vault files no longer in source repos"
  - "syncVault/initVault/cleanOrphans/getVaultStatus: full pipeline orchestration"
affects: [11-04, 11-05]

# Tech tracking
tech-stack:
  added: []
  patterns: [moc-page-generation, manifest-based-change-detection, pipeline-orchestration]
...
key-decisions:
  - "MOC pages use static [[wikilinks]], no Dataview queries for maximum compatibility"
  - "SHA-256 first 16 hex chars for compact content hashing via node:crypto"
  - "Orphan detection is manifest-based: compares manifest paths vs current entry paths"
  - "Sync engine uses shared runPipeline for DRY across sync/init/clean modes"
  - "getVaultStatus collects sources for orphan count but never writes (read-only operation)"
  - "_vault-meta/README.md explains auto-generated nature and source-of-truth repos"

patterns-established:
  - "MOC page as VaultEntry: MOC pages are themselves VaultEntry objects written to 00-MOC/"
  - "Manifest-based incremental sync: hash comparison skips unchanged files"
  - "Pipeline orchestration pattern: resolveConfig -> collect -> transform -> MOC -> write"

requirements-completed: [VAULT-06, VAULT-08, VAULT-09, VAULT-14, VAULT-15]
