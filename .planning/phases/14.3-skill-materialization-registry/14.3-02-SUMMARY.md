...
plan: 02
subsystem: sync
tags: [zod, manifest, schema, validation, typescript, l0-manifest]

# Dependency graph
requires:
  - phase: none
    provides: standalone (wave 1, no dependencies)
provides:
  - ManifestSchema Zod schema for l0-manifest.json validation
  - Manifest TypeScript type for manifest data
  - validateManifest() function for runtime validation
  - createDefaultManifest() for new project bootstrapping
  - readManifest/writeManifest for file I/O
  - generateExampleManifests() documenting shared-kmp-libs and DawSync expected content
affects: [14.3-03-sync-engine, 14.3-05-migration]

# Tech tracking
tech-stack:
  added: [zod (existing dep)]
  patterns: [Zod schema validation, include-all selection model, l2_specific protection]
...
key-decisions:
  - "include-all with excludes is the default selection model (matches git pull semantics)"
  - "l0_source uses z.string().min(1) to reject empty strings"
  - "last_synced uses z.string().datetime() for ISO 8601 validation"
  - "readManifest/writeManifest tests use real temp files instead of ESM module spying (vitest ESM limitation)"

patterns-established:
  - "Manifest schema pattern: Zod schema with .default([]) for optional arrays"
  - "Example manifest pattern: generateExampleManifests() validates at creation time"

requirements-completed: [MATL-02]
