...
plan: 03
subsystem: sync
tags: [sync-engine, materialization, drift-detection, sha256, typescript, vitest, cli]

# Dependency graph
requires:
  - phase: 14.3-01
    provides: "skill-registry.ts with generateRegistry, SkillRegistry, SkillRegistryEntry types"
  - phase: 14.3-02
    provides: "manifest-schema.ts with readManifest, writeManifest, Manifest type, createDefaultManifest"
provides:
  - "sync-engine.ts: resolveSyncPlan, computeSyncActions, materializeFile, syncL0"
  - "SyncPlan, SyncAction, SyncPlanEntry, SyncReport types"
  - "sync-l0-cli.ts CLI entry point with manifest bootstrapping"
  - "skills/sync-l0/SKILL.md for invoking sync from any project"
affects: [14.3-04-adapter-simplification, 14.3-05-migration, 14.3-08-quality-gate]

# Tech tracking
tech-stack:
  added: []
  patterns: [sync-resolution-pattern, hash-diff-pattern, materialization-pattern, l0-sync-header-injection]

key-files:
  created:
    - mcp-server/src/sync/sync-engine.ts
    - mcp-server/tests/unit/sync/sync-engine.test.ts
    - mcp-server/src/sync/sync-l0-cli.ts
    - skills/sync-l0/SKILL.md
  modified:
    - mcp-server/package.json

key-decisions:
  - "Explicit mode resolves entries from checksums keys (opt-in for future use)"
  - "Orphaned file removal uses L0 header safety check (l0_source + l0_hash presence)"
  - "Commands get HTML L0-SYNC comment; skills/agents get YAML frontmatter injection"
  - "CLI bootstraps default manifest on first run (solves chicken-and-egg for new projects)"

patterns-established:
  - "Sync resolution: include-all filters by exclude sets + l2_specific protection"
  - "Hash diff: compare manifest checksums to registry hashes for add/update/remove/unchanged"
  - "Materialization: YAML frontmatter injection for skills/agents, HTML comment header for commands"
  - "CLI manifest bootstrap: create default l0-manifest.json if missing"

requirements-completed: [MATL-03]
