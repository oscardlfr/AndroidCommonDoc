...
plan: 03
subsystem: testing
tags: [konsist, architecture-enforcement, 5-layer, module-isolation, script-parity, skill-validation]

# Dependency graph
requires:
  - phase: 06-konsist-internal-tests-01
    provides: "konsist-tests/ module with ScopeFactory and classpath spike"
provides:
  - "8 architecture violation fixtures demonstrating 5-layer enforcement"
  - "ArchitectureTest with fixture-based violation detection and real-code module isolation"
  - "ScriptParityTest validating SH/PS1 script parity"
  - "SkillStructureTest validating 16 SKILL.md files"
  - "4+ cross-file structural checks (KONS-04) across Plans 02 and 03"
affects: [07-consumer-guard-tests]

# Tech tracking
tech-stack:
  added: []
  patterns: [fixture-based-architecture-testing, with-KoArchitectureCreator-pattern, soft-assertions-for-bulk-validation]
...
key-files:
  created:
    ...
    - konsist-tests/src/test/resources/fixtures/layer-violation/UiScreen.kt
    - konsist-tests/src/test/resources/fixtures/layer-violation/DomainUseCase.kt
    - konsist-tests/src/test/resources/fixtures/layer-violation/ViewModelStub.kt
    - konsist-tests/src/test/resources/fixtures/layer-violation/ModelStub.kt
...
key-decisions:
  - "Konsist assertArchitecture requires with(KoArchitectureCreator) scope pattern for extension function resolution"
  - "Every defined Layer must have at least one file in scope (KoPreconditionFailedException otherwise)"
  - "Konsist throws KoAssertionFailedException (not AssertionError) for architecture violations"
  - "ScriptParityTest intentionally fails to flag 5 orphaned validate-phase* scripts as parity gap"
  - "SkillStructureTest uses SoftAssertions for bulk validation across all 16 skills"

patterns-established:
  - "Fixture-based architecture testing: stub files for all 5 layers with intentional violations for negative tests"
  - "with(KoArchitectureCreator) { scope.assertArchitecture { ... } } pattern for Konsist architecture API"
  - "Slice scope for valid-only subset when testing positive architecture assertions"
  - "SoftAssertions for filesystem validation tests to report all failures at once"

requirements-completed: [KONS-02, KONS-04]
