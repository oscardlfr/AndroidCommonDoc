...
plan: 01
subsystem: testing
tags: [konsist, kotlin, gradle, classpath-isolation, structural-testing]

# Dependency graph
requires:
  - phase: 05-tech-debt-foundation
    provides: "Clean baseline with no false-positive quality gate noise"
provides:
  - "konsist-tests/ standalone JVM module with Konsist 0.17.3"
  - "ScopeFactory utility with canary assertions for all scope creation"
  - "Classpath spike proving Konsist 0.17.3 + Kotlin 2.3.10 compatibility"
  - "UP-TO-DATE bypass (KONS-05) ensuring tests always re-execute"
affects: [06-02-PLAN, 06-03-PLAN, 07-consumer-guard-tests]

# Tech tracking
tech-stack:
  added: [konsist-0.17.3, kotlin-compiler-embeddable-2.0.21]
  patterns: [isolated-test-module, scope-factory-canary, relative-path-scope-resolution]
...
key-decisions:
  - "Relative paths for Konsist scopeFromDirectory (absolute paths cause path duplication on Windows)"
  - "Validate directory existence via java.io.File before passing to Konsist"
  - "No Detekt plugin in konsist-tests module to avoid kotlin-compiler-embeddable classpath conflict"

patterns-established:
  - "ScopeFactory singleton: centralized scope creation with canary assertions for non-empty results"
  - "Relative path resolution: use relative paths with Konsist, validate via File separately"
  - "Standalone module pattern: own settings.gradle.kts, kotlin-jvm only, no composite build"

requirements-completed: [KONS-01, KONS-05]
