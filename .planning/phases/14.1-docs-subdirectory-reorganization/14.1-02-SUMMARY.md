...
tags: [markdown, frontmatter, mcp, typescript, vitest, category, subdirectory, reorganization]
...
provides:
  - "All 42 L0 docs with category frontmatter in domain-based subdirectories"
  - "validate-doc-structure MCP tool for category-subdir alignment validation"
  - "docs/README.md auto-generated index with subdirectory table"
  - "Cross-references updated for new subdirectory structure"
affects: [14.1-03, 14.1-04, 14.1-05, 14.1-06]

# Tech tracking
tech-stack:
  added: []
  patterns: [category-frontmatter-classification, domain-based-subdirectory-organization, cross-reference-relative-paths]
...
key-decisions:
  - "viewmodel-* docs placed in ui/ subdirectory alongside ui-screen-* (both are UI-layer patterns)"
  - "claude-code-workflow.md category changed from 'workflow' to 'guides' per plan mapping"
  - "archive/ directory skipped during validation (consistent with scanner behavior from Plan 01)"
  - "Docs at root level with category field treated as error (must be in matching subdirectory)"
  - "Cross-references use relative paths (../testing/testing-patterns.md) not absolute paths"

patterns-established:
  - "Category frontmatter field determines subdirectory placement for all L0 docs"
  - "validate-doc-structure exports validateDocsDirectory + generateReadmeIndex for direct testing"
  - "MCP tool registration pattern with cross-project discovery via discoverProjects()"

requirements-completed: [REORG-01, REORG-04, REORG-05]
