...
plan: 01
subsystem: tooling
tags: [mcp, validation, l0_refs, size-limits, frontmatter, vitest]

requires:
  - phase: 14.1-docs-subdirectory-reorganization
    provides: Scanner with recursive subdirectory discovery and category extraction
provides:
  - PatternMetadata.l0_refs field for cross-layer reference tracking
  - checkSizeLimits function (500-line max, 100-line hub max, 150-line section warning)
  - validateL0Refs function for l0_refs slug resolution
  - frontmatterCompleteness scoring function (0-10 scale)
affects: [14.2-02, 14.2-03, 14.2-04, 14.2-05, 14.2-06, 14.2-07, 14.2-08, 14.2-09]

tech-stack:
  added: []
  patterns: [TDD red-green for MCP tool extensions, size limit validation, cross-layer reference validation]
...
key-decisions:
  - "Hub doc detection uses '## Sub-documents' heading marker (consistent with existing sub-doc pattern)"
  - "Frontmatter completeness scores 10 fields: scope, sources, targets, slug, status, layer, category, description, version, last_updated"
  - "Empty arrays count as not present for completeness scoring (encourages meaningful content)"
  - "Integration test verifies hub detection works correctly rather than asserting zero violations (existing hub docs exceed 100 lines)"

patterns-established:
  - "Size limit validation: 500 absolute max, 100 hub max, 150 section warning, archive excluded"
  - "l0_refs extraction follows existing scanner array pattern (Array.isArray guard)"
