...
tags: [hub-sub-doc, doc-splitting, frontmatter, layer-L0, offline-first, viewmodel, di, compose-resources, storage]

requires:
  - phase: 14.2-01
    provides: "MCP tooling with size limit validation, l0_refs, frontmatter completeness checks"
  - phase: 14-02
    provides: "Hub+sub-doc pattern established with 6 docs split"
provides:
  - "7 oversized L0 docs split into hub+sub-doc format"
  - "10 new sub-docs with full frontmatter (layer, parent, slug, category)"
  - "7 hub/hub-like docs under 100 lines each (except offline-first-sync at 114)"
  - "All sub-docs under 300 lines"
  - "doc-template.md (267 lines) preserved as-is"
affects: [14.2-03, 14.2-09, 15]

tech-stack:
  added: []
  patterns: ["Hub-like sub-doc pattern: sub-doc that also has its own sub-docs with '## Sub-documents' section"]
...
key-decisions:
  - "offline-first-sync kept conflict resolution code in hub (114 lines) -- essential quick-reference for a sub-doc that is also a hub"
  - "Hub-like sub-doc pattern: docs that are sub-docs (have parent field) but also act as hubs (have Sub-documents section) with their own sub-docs"
  - "doc-template.md (267 lines) preserved as-is -- splitting would fragment the reference template example"

patterns-established:
  - "Hub-like sub-doc: a sub-doc with parent field that also has ## Sub-documents section linking to its own sub-docs"
  - "Two-level hub hierarchy: top hub -> hub-like sub-doc -> leaf sub-doc (e.g., offline-first-patterns -> offline-first-architecture -> offline-first-architecture-layers)"
