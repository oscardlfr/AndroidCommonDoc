...
tags: [claude-md, copilot-adapter, smoke-test, integration-test, ecosystem-validation]

requires:
  - phase: 15-03-claude-md-ecosystem-rewrites
    provides: All 4 CLAUDE.md files rewritten with identity headers and delegation chain
provides:
  - Copilot adapter generating .github/copilot-instructions.md from CLAUDE.md as SSOT
  - Integration tests validating all 4 CLAUDE.md files on disk (template, coverage, cross-file)
  - Canonical rule smoke test confirming 90%+ coverage per layer
  - Human-verified CLAUDE.md ecosystem alignment
affects: [future CLAUDE.md maintenance, Copilot instruction generation pipeline]

tech-stack:
  added: []
  patterns: [claude-md-copilot-generation, canonical-coverage-smoke-test, cross-file-integration-validation]

key-files:
  created:
    - adapters/claude-md-copilot-adapter.sh
    - mcp-server/tests/integration/claude-md-validation.test.ts
    - setup/copilot-templates/copilot-instructions-from-claude-md.md
  modified:
    - adapters/generate-all.sh
    - C:/Users/34645/.claude/CLAUDE.md

key-decisions:
  - "Copilot adapter uses Python3 inline script (matching existing adapter pattern) to flatten L0 + project CLAUDE.md into Copilot format"
  - "Integration tests read real filesystem paths (cross-platform via path.resolve) for all 4 CLAUDE.md locations"
  - "Canonical coverage uses keyword heuristic matching from validate-claude-md tool -- 90%+ threshold per layer"
  - "Post-checkpoint fix: restored ../shared-kmp-libs path in Build Patterns (L0 global + Copilot output) since it is a real Gradle filesystem path, not a project name reference"

patterns-established:
  - "CLAUDE.md as SSOT for Copilot: single adapter generates Copilot instructions from CLAUDE.md files instead of maintaining separate rule sources"
  - "Integration test pattern: real filesystem reads for cross-project validation with path.resolve for Windows/Unix portability"

requirements-completed: [CLAUDE-07]
