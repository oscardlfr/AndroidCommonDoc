...
tags: [claude-md, mcp-tool, validation, vitest, ecosystem-alignment]

requires:
  - phase: 15-01-canonical-rule-extraction
    provides: canonical-rules.json with 66 rules across 14 categories
provides:
  - validate-claude-md MCP tool with 7 validation dimensions
  - 26 unit tests covering template structure, coverage, references, overrides, versions, duplicates
  - Tool registered in MCP index (16 tools total)
affects: [15-03 CLAUDE.md rewrites, 15-04 smoke tests and Copilot adapter]

tech-stack:
  added: []
  patterns: [keyword-heuristic-rule-matching, circular-reference-detection, cross-file-duplicate-detection]
...
key-decisions:
  - "Keyword heuristic matching for canonical coverage: extract 2-4 distinctive keywords per rule, require half to match (avoids brittle exact string matching)"
  - "Developer Context section exempt from circular reference detection (user-scoped, allowed to mention project names)"
  - "L0-global (~/.claude/CLAUDE.md) exempt from identity header requirement (it is the hierarchy root)"
  - "Version patterns use named regex patterns (Koin, Kotlin, AGP, etc.) mapped to versions-manifest.json keys"
  - "Cross-file duplicate detection uses normalized lowercase comparison of rule-like lines (lines starting with - and >20 chars)"

patterns-established:
  - "CLAUDE.md validation pipeline: template structure > line count > canonical coverage > circular refs > overrides > versions > duplicates"
  - "Exported internal validation functions for unit testability (validateTemplateStructure, validateCanonicalCoverage, etc.)"
  - "TDD workflow for MCP tools: RED (26 failing tests) > GREEN (implementation) > commit per phase"

requirements-completed: [CLAUDE-08, CLAUDE-06]
