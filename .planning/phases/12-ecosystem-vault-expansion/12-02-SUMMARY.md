...
plan: 02
subsystem: vault
tags: [typescript, vault-types, project-config, layer-hierarchy, obsidian]

# Dependency graph
requires:
  - phase: 11-obsidian-documentation-hub
    provides: "Original vault types, config, and sync pipeline"
  - phase: 12-00
    provides: "Test stubs for vault unit tests"
provides:
  - "ProjectConfig and SubProjectConfig type contracts"
  - "VaultConfig v1 schema with version field"
  - "VaultSourceType with 'architecture' variant"
  - "Layer-required VaultSource and VaultEntry interfaces"
  - "Config loader with old format detection and migration"
  - "getDefaultGlobs and getDefaultExcludes utility functions"
...
tech-stack:
  added: []
  patterns:
    - "Schema versioning with version:1 literal type for future migrations"
    - "Old format detection with clean break (no backward compat)"
    - "ProjectConfig validation filtering invalid entries"
...
key-decisions:
  - "Clean break from old string[] projects format -- detected and defaults returned with warning"
  - "version:1 literal type field for future migration support"
  - "getDefaultGlobs accepts layer parameter (reserved for future layer-specific defaults)"
  - "ProjectConfig validation skips invalid entries with warning instead of throwing"

patterns-established:
  - "Schema versioning: version literal type field on config interfaces for forward-compatible migrations"
  - "Format detection: isOldFormat helper checks missing version + string[] projects for clean schema transitions"

requirements-completed: [ECOV-05, ECOV-07]
