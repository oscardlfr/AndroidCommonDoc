...
plan: 01
subsystem: docs
tags: [audit, wakethecave, classification, ai-readiness, l0-promotion]

# Dependency graph
requires:
  - phase: 12-ecosystem-vault
    provides: L0/L1/L2 hierarchy model, vault collection infrastructure
provides:
  - audit-manifest-wakethecave.json with per-file classification of all 209 WakeTheCave docs
  - WakeTheCave doc health advisory recommendations
  - Overlap mapping between WakeTheCave docs and L0 AndroidCommonDoc patterns
affects: [13-04 audit report assembly, 14 template design]

# Tech tracking
tech-stack:
  added: []
  patterns: [per-file audit manifest schema with AI-readiness scoring]
...
key-decisions:
  - "0 L0 promotion candidates: all WakeTheCave docs contain project-specific context (device types, Hue integration, smart home domain) that makes them L2; the generic patterns they reference are already covered in AndroidCommonDoc L0 docs"
  - "Conservative threshold applied correctly: testing/error-handling/viewmodel patterns in WakeTheCave derive from official docs but contain WakeTheCave-specific examples (mockk, WakeTheCave theme, DeviceCapability types) making them L2 not L0"
  - "Version detection via prose patterns (not Gradle dependency strings): WakeTheCave docs reference versions only in build.gradle snippets, not in prose, so no stale version refs detected"

patterns-established:
  - "Audit manifest JSON schema: project, path, layer, audit_mode, files[], summary{}"
  - "AI-readiness 0-5 scoring: frontmatter, section-size, subdivision, stale-refs, actionable-content"
