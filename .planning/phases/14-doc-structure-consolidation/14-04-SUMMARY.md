...
plan: 04
subsystem: docs
tags: [skills, agents, commands, workflow, lighthouse, web-quality, kmp, claude-code]

requires:
  - phase: 14-01
    provides: "Prerequisites template, types, versions-manifest, verification"
provides:
  ...
  - "1 workflow doc extracted to L0"
affects: [14-09, 15-claude-md-rewrite]

tech-stack:
  added: []
  patterns:
    - "L0 Generic Agent pattern: provenance header with $PROJECT_ROOT parameterization"
    - "L0 Generic Command pattern: provenance header with $ARGUMENTS native expansion"
    - "Skill directory structure: SKILL.md + references/ subdirectory"
...
key-files:
  created:
    ...
    - ".claude/commands/feature-audit.md"
    ...
    - ".claude/commands/start-track.md"
    - ".claude/commands/sync-roadmap.md"
    - ".claude/commands/sync-tech-versions.md"
    - ".claude/commands/unlock-tests.md"
    - ".claude/commands/verify-migrations.md"
    - "docs/claude-code-workflow.md"
  modified: []

key-decisions:
  - "Classified 7 commands as DawSync-specific (test-m4l, deploy-web, lint-web, run-clean, nuke-builds, roadmap, validate-strings) -- kept at L2 only"
  - "16 existing commands in AndroidCommonDoc already matched DawSync counterparts -- no duplication needed"
  - "Skills copied as-is since they contain zero project-specific references (all generic Lighthouse/web-quality patterns)"
  - "Agent parameterization uses $PROJECT_ROOT and {placeholder} patterns for project-specific paths"
  - "Provenance comments (Promoted from DawSync) kept in HTML comments as allowed by verification criteria"

patterns-established:
  - "L0 agent promotion: Replace hardcoded paths with $PROJECT_ROOT, domain terms with {placeholder}, add provenance header"
  - "L0 command promotion: Replace project-specific workflow references with generic patterns, keep $ARGUMENTS expansion"
