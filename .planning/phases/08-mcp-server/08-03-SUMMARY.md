...
plan: 03
subsystem: mcp
tags: [typescript, mcp-sdk, execFile, rate-limiter, validation-tools, vitest]

# Dependency graph
requires:
  - phase: 08-01
    provides: Server factory, types (ValidationResult, ToolResult), paths utility, logger, stub registerTools
provides:
  - Cross-platform script runner utility (execFile with NO_COLOR, timeout)
  - Sliding window rate limiter (30 calls/60s)
  - 5 individual validation tools (check-freshness, verify-kmp, check-version-sync, script-parity, setup-check)
  - validate-all meta-tool combining all validation gates
  - rate-limit-status utility tool
  - Tool registration aggregator replacing stub
affects: [08-04]

# Tech tracking
tech-stack:
  added: []
  patterns: ["rate-limit-guard-pattern", "script-runner-execFile", "tool-registration-with-limiter", "validate-all-gate-aggregation"]

key-files:
  created:
    - mcp-server/src/utils/script-runner.ts
    - mcp-server/src/utils/rate-limiter.ts
    - mcp-server/src/utils/rate-limit-guard.ts
...
key-decisions:
  - "Rate limiter injected per-tool via optional parameter rather than server proxy (SDK registerTool has complex overload signatures incompatible with proxying)"
  - "script-parity tool is a direct implementation (compares sh/ vs ps1/ directories) rather than wrapping a script"
  - "ToolResponse type uses Record<string, unknown> intersection for MCP SDK handler compatibility"
  - "Rate-limit-guard as separate utility module for clean single-responsibility separation"

patterns-established:
  - "Rate-limit guard pattern: checkRateLimit(limiter, toolName) returns error response or null, called at handler entry"
  - "Script runner pattern: runScript(baseName, args, rootDir) with NO_COLOR=1, ANDROID_COMMON_DOC env, configurable timeout"
  - "Tool registration with limiter: each register function accepts optional RateLimiter, index.ts creates shared instance"
  - "Gate aggregation pattern: validate-all runs gates sequentially, collects ToolResult[], computes overall PASS/FAIL"
