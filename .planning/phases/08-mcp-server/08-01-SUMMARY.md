...
plan: 01
subsystem: mcp
tags: [typescript, mcp-sdk, stdio, vitest, eslint, node]

# Dependency graph
requires: []
provides:
  - MCP server project skeleton with TypeScript compilation
  - Server factory pattern (createServer) testable via InMemoryTransport
  - stdio entry point for Claude Code subprocess
  - stderr-only logger utility
  - Toolkit path resolution (env var + fallback)
  - Shared ValidationResult/ToolResult types
  - Stub registration functions (registerResources, registerTools, registerPrompts)
affects: [08-02, 08-03, 08-04]

# Tech tracking
tech-stack:
  added: ["@modelcontextprotocol/sdk@1.27.1", "zod@3", "typescript@5.9.3", "vitest@3.2.4", "eslint@9", "@typescript-eslint/parser@8", "prettier@3"]
  patterns: ["server-factory-pattern", "stderr-only-logging", "stub-registration", "InMemoryTransport-testing"]
...
key-decisions:
  - "@typescript-eslint/parser added as devDependency for ESLint 9 TypeScript parsing"
  - "InMemoryTransport from @modelcontextprotocol/sdk/inMemory.js confirmed as test transport path"
  - "Integration tests verify handshake and server info rather than empty list operations (SDK only declares capabilities when handlers are registered)"

patterns-established:
  - "Server factory pattern: createServer() separated from transport for testability"
  - "stderr-only logging: all console.error, no console.log, enforced by ESLint no-console rule"
  - "ES Module imports with .js extensions throughout (required by Node16 module resolution)"
  - "Stub registration pattern: registerResources/Tools/Prompts functions in dedicated modules"
