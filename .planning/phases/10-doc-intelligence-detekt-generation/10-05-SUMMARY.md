...
plan: 05
subsystem: mcp
tags: [mcp-tools, detekt, content-ingestion, pattern-matching, url-fetch, rate-limiting]

requires:
  - phase: 10-03
    provides: "monitor-sources MCP tool pattern, review state infrastructure"
  - phase: 10-04
    provides: "writeGeneratedRules pipeline (writer.ts), GenerationResult interface"
provides:
  - "generate-detekt-rules MCP tool for triggering rule generation from frontmatter"
  - "ingest-content MCP tool for URL/pasted content pattern extraction"
  - "11 registered MCP tools (complete Phase 10 tool surface)"
affects: [10-06, 10-07, 11-notebooklm-integration-skill]

tech-stack:
  added: []
  patterns: [url-fetch-with-timeout, keyword-extraction, content-pattern-matching, suggest-and-approve-flow]
...
key-decisions:
  - "dry_run defaults to true for generate-detekt-rules (safety-first for code generation)"
  - "Keyword frequency extraction (count>1) for content matching against pattern doc metadata"
  - "URL fetch with 15-second timeout and AbortController for ingest-content"
  - "Content ingestion returns suggestions with recommended_action (update/review/new_doc), never auto-applies"

patterns-established:
  - "URL graceful degradation: unfetchable URLs return structured response with paste prompt"
  - "Content pattern matching: keyword extraction -> metadata field matching -> relevance scoring"
  - "Suggest-and-approve flow: all content ingestion outputs are suggestions requiring user review"

requirements-completed: [DOC-04, DOC-07]
