...
plan: 03
subsystem: registry
tags: [resolver, layer-priority, project-discovery, settings-gradle, typescript, vitest]

requires:
  - phase: 09-pattern-registry-discovery
    provides: "Registry types (RegistryEntry, Layer), scanner (scanDirectory), paths utility"
provides:
  - "Three-layer resolver (resolvePattern, resolveAllPatterns, resolveAllPatternsWithExcludes) with L1>L2>L0 priority"
  - "Project auto-discovery (discoverProjects, ProjectInfo) from settings.gradle.kts includeBuild paths"
  - "Path helpers for L1 and L2 directories (getL1DocsDir, getL2Dir, getL2DocsDir)"
affects: [09-04, 09-05, 09-06]

tech-stack:
  added: []
  patterns: [three-layer resolution with full replacement, sibling-directory project discovery, YAML fallback config]
...
key-decisions:
  - "L1>L2>L0 full replacement semantics -- no merging, highest-priority layer wins per slug"
  - "resolveAllPatternsWithExcludes filters by sources intersection for doc-level excludes"
  - "Project discovery scans sibling dirs of toolkit root, falls back to projects.yaml"
  - "HOME/USERPROFILE env vars used for cross-platform home directory in tests"

patterns-established:
  - "Three-layer resolution: L1 (project) > L2 (user) > L0 (base) with full replacement per slug"
  - "Sibling-directory discovery: scan parent dir for settings.gradle.kts with includeBuild regex"
  - "YAML fallback config: ~/.androidcommondoc/projects.yaml for manual project registration"

requirements-completed: [REG-03, REG-04]
