...
plan: 06
subsystem: registry
tags: [migration, frontmatter, L0, L1, dawsync, error-handling]

requires:
  - phase: 09-pattern-registry-discovery
    plan: 01
    provides: "YAML frontmatter parser, directory scanner, registry types, 9 L0 docs with frontmatter"
provides:
  - "New L0 error-handling-patterns doc (Result type, CancellationException, DomainException hierarchy)"
  - "DawSync L1 override directory (.androidcommondoc/docs/) with project-specific patterns"
  - "DawSync domain patterns doc (producer/consumer, DAW guardian, freemium gating, action queue)"
affects: [09-03, 09-04, 09-05]

tech-stack:
  added: []
  patterns: [L0/L1 layer separation, DawSync doc migration, error handling pattern doc]

key-files:
  created:
    - docs/error-handling-patterns.md
    - (external) DawSync/.androidcommondoc/docs/dawsync-domain-patterns.md
    - (external) DawSync/.androidcommondoc/README.md
  modified: []

key-decisions:
  - "Error handling promoted to L0: cross-cutting concern (Result type, CancellationException, DomainException hierarchy) referenced by 5+ DawSync agents but not covered by existing L0 docs"
  - "Single L1 domain patterns doc consolidates producer/consumer, DAW guardian, freemium gating, and action queue (all DawSync-specific)"
  - "No L1 overrides of existing L0 slugs needed: DawSync agents reference existing L0 patterns (testing, viewmodel, UI) without project-specific modifications"

patterns-established:
  - "L1 override directory convention: .androidcommondoc/docs/ in consumer project root"
  - "L1 docs use same YAML frontmatter schema as L0 for scanner compatibility"
