...
plan: 04
subsystem: mcp
tags: [mcp, registry, scanner, pattern-discovery, zod, dynamic-resources]

# Dependency graph
requires:
  - phase: 09-01
    provides: "Registry scanner (scanDirectory) and types (RegistryEntry, PatternMetadata)"
  - phase: 09-03
    provides: "Layer resolver (resolveAllPatterns) and project discovery (discoverProjects)"
provides:
  - "Dynamic registry-aware doc resource registration (no hardcoded lists)"
  - "find-pattern MCP tool for metadata-based pattern search"
  - "Async createServer factory for all test and production use"
affects: [09-05, mcp-server, docs-resources]

# Tech tracking
tech-stack:
  added: []
  patterns: ["async createServer factory", "backward-compatible slug aliases", "metadata tokenized search"]
...
key-decisions:
  - "Backward-compatible slug alias map (enterprise-integration -> enterprise-integration-proposal) instead of renaming file"
  - "Async createServer cascading from async registerDocResources, requiring update to 13 files total"
  - "Query tokenization with case-insensitive substring matching across scope/sources/targets"
  - "Deduplicate matches by slug in find-pattern to prevent duplicates from cross-project search"

patterns-established:
  - "Async server factory: createServer() returns Promise<McpServer> for registry scan at startup"
  - "Slug aliases: SLUG_ALIASES map for backward-compatible URI schemes when filenames differ"
  - "Metadata search: tokenize query, match any token against any metadata field value"
