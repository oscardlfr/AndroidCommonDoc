...
plan: 01
subsystem: infra
tags: [setup-scripts, env-var-guard, bash, powershell, copilot-prompts]

# Dependency graph
requires: []
provides:
  - "ANDROID_COMMON_DOC env var guard on all 8 consumer-facing scripts"
  - "Standalone install-copilot-prompts.sh/ps1 delivering all copilot artifacts"
  - "Clean delegation in setup-toolkit.sh/ps1 Step 4 (no inline duplication)"
affects: [05-02-PLAN]
...
tech-stack:
  added: []
  patterns:
    - "Two-phase env var guard: check set, then check path exists"
    - "Guard placement after --set-env processing for scripts that can self-provision"
...
key-decisions:
  - "Env var guard placed after --set-env processing to allow first-time provisioning"
  - "Pre-existing ((INSTALLED++)) set -e bug logged to deferred-items.md, not fixed (out of scope)"

patterns-established:
  - "Env var guard pattern: two-phase check with actionable export command in error message"
  - "SH/PS1 parity: every SH guard has an equivalent PS1 guard with matching text"
